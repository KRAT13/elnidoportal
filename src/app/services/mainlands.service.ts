...
import { Mainlands } from '../models';

@Injectable({
  providedIn: 'root'
})
export class MainlandsService {
    
  private readonly initialMainlands: Mainlands[] = [
    {
        code: 'A-BL',
        name: 'Big Lagoon',
  ...
  private readonly _continents = signal<Mainlands[]>(this.initialMainlands);
  ...
  getContinentByCode(code: string): Mainlands | undefined {
    return this._continents().find(c => c.code === code.toUpperCase());
  }

  addContinent(island: Mainlands): void {
